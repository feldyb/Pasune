...
async function exportDocx() {
  ...
  try {
    ...
    // Încearcă Web Share API (funcționează în WebView Android și Chrome mobile)
    if (navigator.canShare && navigator.canShare({ files: [new File([blob], filename, {type: docxType})] })) {
      const file = new File([blob], filename, { type: docxType });
      await navigator.share({ files: [file], title: 'Fișe Pășune' });
    } else {
      // Fallback: download clasic (Chrome desktop, Firefox)
