...
async function exportDocx() {
  ...
  try {
    ...
    const filename = `fise_pasune_${new Date().toISOString().slice(0,10)}.docx`;
    const docxType = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';

    // Varianta 1: File System Access API — dialog "Salvează ca..." cu alegerea locației
    if (window.showSaveFilePicker) {
      try {
        const handle = await window.showSaveFilePicker({
          suggestedName: filename,
          types: [{ description: 'Word Document', accept: { [docxType]: ['.docx'] } }]
        });
        const writable = await handle.createWritable();
        await writable.write(blob);
        await writable.close();
        // toast-ul de succes vine după
      } catch(e2) {
        if (e2.name !== 'AbortError') throw e2;
        return; // utilizatorul a anulat dialogul
      }

    // Varianta 2: Web Share API — meniu Android (WebView, Chrome mobile)
    } else if (navigator.canShare && navigator.canShare({ files: [new File([blob], filename, {type: docxType})] })) {
      const file = new File([blob], filename, { type: docxType });
      await navigator.share({ files: [file], title: 'Fișe Pășune' });

    // Varianta 3: Download clasic (fallback)
    } else {
  ...
  function buildFisa(f) {
    ...
    if (f.catFolos === 'Pășune împădurită') {
      ...
      function piD(w, val, o={}) {
        return new TableCell({ borders:brdP, width:{size:w,type:WidthType.DXA},
          margins:padSm, verticalAlign:VerticalAlign.CENTER, columnSpan:o.span||1,
      ...
      function row4(l1,v1,l2,v2) {
        return new TableRow({height:{value:400,rule:'exact'}, children:[
          ...
          new TableCell({borders:brdP,width:{size:c4f[1],type:WidthType.DXA},margins:padSm,
          ...
          new TableCell({borders:brdP,width:{size:c4f[3],type:WidthType.DXA},margins:padSm,
